...
const Header = ({ setShowLogin }) => {
  ...
  return (
    <header
      ...
      } fixed w-full mx-auto top-0 left-0 ring-0 py-4 z-30 transition-all max-padd-container flexBetween `}
    >
      {/* logo */}
      <Link to={'/'}>
        <img src={logo} alt='logoImg' height={100} width={120} />
      </Link>
      ...
      <div className='flexBetween gap-x-3 sm:gap-x-8'>
        ...
        <Link to={'/cart'} className='flex relative'>
          <GiShoppingBag
            className='text-[22px] text-white bg-secondary h-9 w-9 p-2 
        rounded-xl '
          />
        ...
        {!token ? (
          ...
          <div className='group relative'>
            ...
            <ul className='bg-primary shadow-sm p-3 w-24 ring-1 ring-slate-900/15 rounded absolute right-0 group-hover:flex flex-col hidden '>
              ...
              <hr className='my-2' />
              <li onClick={logout} className='flexBetween gap-x-2 cursor-pointer'>
